...
//           onPress={() => navigation.navigate('TodoDetails', { todoId: item._id! })}
...
import { Ionicons } from '@expo/vector-icons';
import { Swipeable } from 'react-native-gesture-handler';
import { fetchTodos, deleteTodo, getTodoStats } from '../../api/Todoapi';
import { useAuth } from '../../context/AuthContext';
import type { Todo } from '../../api/Todoapi';
import { NativeStackNavigationProp } from '@react-navigation/native-stack';
import { TodoStackParamList } from '../../navigation/TodoStackNavigator';
...
const STATUS_TABS = ['All', 'Todo', 'In-Progress', 'Done'];
const VIEW_MODES = ['All', 'Solo', 'Shared'];
const PRIORITY_COLORS = {
  low: '#4ade80',
  medium: '#facc15',
  high: '#f87171',
...
const allCategories = useMemo(() => {
  const categories = todos.map(t => t.category).filter((c): c is string => !!c);
  return [...new Set(categories)]; // Deduplicate and convert back to array
}, [todos]);

  const isShared = (todo: Todo) => (todo.participants?.length ?? 0) > 1;
  ...
  useEffect(() => {
    loadTodos();
    saveFilters();
  }, [selectedTab, selectedCategory, viewMode]);
  ...
  const loadTodos = async () => {
    ...
    try {
      ...
      const [todosResponse, stats] = await Promise.all([
        fetchTodos(statusParam ? { status: statusParam } : {}, token),
        getTodoStats(token),
      ]);

      const userId = user?.id;
      const updatedTodos = todosResponse.map(todo => ({
        ...todo,
        userDone: todo.completions?.some(c => c.userId?._id === userId) || false,
        isShared: isShared(todo),
      }));
  ...
  const filteredTodos = useMemo(() => {
    let filtered = [...todos];
    if (viewMode === 'Solo') filtered = filtered.filter(t => !t.isShared);
    if (viewMode === 'Shared') filtered = filtered.filter(t => t.isShared);
    if (selectedCategory) filtered = filtered.filter(t => t.category === selectedCategory);
    return filtered;
  }, [todos, selectedCategory, viewMode]);

  const renderTodo = ({ item }: { item: EnhancedTodo }) => (
    <Animated.View style={{ opacity: fadeAnim }}>
      <Swipeable
        renderRightActions={() => (
          <TouchableOpacity
      ...
      >
        <TouchableOpacity
          ...
          style={[
            ...
            item.isShared && styles.sharedTodo,
          ]}
        >
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <Ionicons
            ...
            <View>
              <Text style={[
                styles.todoTitle,
                isExpired(item) && styles.expiredText,
                item.userDone && styles.completedText
              ]}>
                {item.title}
              </Text>
              <Text style={styles.todoMeta}>
              ...
              <Text style={styles.todoMeta}>
                ...
                <Text style={{ color: item.isShared ? '#facc15' : '#888' }}>
                  {item.isShared ? ' Shared' : ' Solo'}
                </Text>
              </Text>
            </View>
          </View>
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={renderTodo}
          refreshControl={
            <RefreshControl refreshing={refreshing} onRefresh={loadTodos} colors={["#26dbc3"]} />
          }
        />
      )}
