...
type CombinedNavigationProp = DrawerNavigationProp<RootDrawerParamList> &
...
export default function FABMenu() {
  ...
  // Separated logic to avoid mutation warnings
  const playAll = () => {
  ...
  const options = [
    {
      icon: require('../assets/lottie/plus.json'),
      label: 'New Challenge',
      onPress: () => {
        toggle();
    ...
    {
      icon: require('../assets/lottie/todo.json'),
      label: 'Todos',
      onPress: () => {
        toggle();
        navigation.reset({ index: 0, routes: [{ name: 'TodoModule' }] });
      },
    },
  ];
          ...
          <LottieView
            ...
            source={
                isOpen
                       ...
                       : require('../assets/lottie/plus.json')
            }
            autoPlay={false}
            loop
